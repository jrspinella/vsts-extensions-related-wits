import { ActionsHub } from "../Actions/ActionsHub";
...
export class RelatedWorkItemsStore extends BaseStore<WorkItem[], WorkItem, number> {
    ...
    public getItem(id: number): WorkItem {
        if (this.items) {
            const index = ArrayUtils.findIndex(this.items, w => w.id === id);
    ...
    protected initializeActionListeners() {
        ActionsHub.UpdateWorkItemInStore.addListener(workItem => {
            if (workItem) {
                const index = ArrayUtils.findIndex(this.items, w => w.id === workItem.id);
                if (index !== -1) {
